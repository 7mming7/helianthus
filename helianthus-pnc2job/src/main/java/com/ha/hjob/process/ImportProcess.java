...
import com.ha.exception.ProcessFailureException;
import com.ha.log.LogGobbler;
...
public class ImportProcess {
    ...
    public ImportProcess(final List<String> cmd, final Map<String, String> env,
    ...
    public boolean softKill(final long time, final TimeUnit unit)
        ...
        if (processId != 0 && isStarted()) {
            try {
                String cmd =
                        String.format("%s %d", KILL_COMMAND, processId);
    ...
    public void hardKill() {
        ...
        if (isRunning()) {
            if (processId != 0) {
                try {
                    String cmd =
                            String.format("%s -9 %d", KILL_COMMAND, processId);
                    Runtime.getRuntime().exec(cmd);
                } catch (IOException e) {
                    logger.error("Kill attempt failed.", e);
                }
